...
int jacobi(double** A, double* b, double* x_,int size, int max_it, double epsilon, int num_t);
...
int main(int argc, char *argv[])
{
  ...
  switch(argc)
  {
  	case 1: // no arguments
  		...
  		printf("0 input arguments: I am using demi data, number of threads %d\n", num_t);
		break;
	case 3: // 2 arguments
		filename_A = argv[1];
		filename_b = argv[2];
		num_t = omp_get_max_threads();
		printf("2 input arguments: I am using A matrix from '%s' file and b vector from '%s' file, number of threads %d\n", filename_A, filename_b, num_t);
		break;
	case 4: // 2 arguments
		filename_A = argv[1];
		filename_b = argv[2];
		num_t = atoi(argv[3]);
		printf("3 input arguments: I am using A matrix from '%s' file and b vector from '%s' file, number of threads %d\n", filename_A, filename_b, num_t);
		break;
	default:
...
// chceck num of treads
	if(num_t > omp_get_max_threads() | num_t < 1)
	{
		printf("ERROR:\nNumber of threads sould be integer number between <1 ... %d>\n", omp_get_max_threads());
		exit(0);
	}
...
//  double epsilon = 0.000001;
  ...
  k = jacobi(A, b, x_, size, MAX_IT, EPSILON, num_t);
...
//	    		printf("Number of threads %d\n",omp_get_num_threads());
			...
			for(i=0; i<rows; i++)
			{
				serr += pow(x_[i]-x_0[i],2); //[BUG FIXED]
				x_0[i]= x_[i];
			}
		}//omp parallel
		rserr = sqrt(serr);
